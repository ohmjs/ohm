...
- On a terminal node (e.g., a node produced by the parsing expression `"hello"`), use the semantic action named '_terminal'. If the action dictionary does not have a property with that name, the default action returns the node's *primitive value*.
