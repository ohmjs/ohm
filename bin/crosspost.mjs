...
const USAGE = `Usage: crosspost.mjs [command] [dir]
...
async function runPost() {
  ...
  if (allStrategies.length === 0) {
    console.error(
      "No services configured. Set env vars for Bluesky and/or Mastodon."
    );
    process.exit(1);
  }
  ...
  for (const post of pending) {
    ...
    try {
      ...
      if (post.image) {
        ...
        opts.images = [
          { data: new Uint8Array(readFileSync(post.image)), alt: post.image_alt ?? basename(post.image) },
        ];
      }
...
async function promptImage() {
  ...
  const image_alt = await p.text({
    message: "Alt text",
    placeholder: basename(image),
  });
  if (p.isCancel(image_alt)) return null;

  return { image, image_alt: image_alt || basename(image) };
}
...
async function editPost(posts) {
  ...
  const message = await readMultiline(
    p.log.step("New message (replaces current)") ?? ""
  );
  if (!message) {
